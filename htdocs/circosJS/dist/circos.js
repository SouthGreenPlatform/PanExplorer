...
/***/ function(module, exports) {
	...
	/***/ function(module, exports, __webpack_require__) {
		...
		function render() {
			...
			var svg = circos.svg.attr('width', circos.conf.width).attr('height', circos.conf.height).attr('id','svgdemo');
			...
			_createClass(Chords, [{
				...
				value: function renderChords(parentElement, name, conf, data, instance, getCoordinates) {
					...
					var link = track.selectAll('.chord').data(data).enter().append('path').attr('class', 'chord').attr('d', (0, _d3Chord.ribbon)().source(function (d) {
						...
						$("#modalContent").text("")
						if($("#userUrl").val() !== ""){
							var trackdiv = document.getElementById("modalContent");
							...
							label.appendChild(document.createTextNode("Select Chromosome"));
							var divcontent = document.createElement("div");
							divcontent.setAttribute("class","col-lg-7");
							var input = document.createElement("select");

							input.setAttribute("id","chrselect");
							input.setAttribute("class", "form-control");
							input.setAttribute("onchange",'var linkinput; var link; document.getElementById("urlFinal").innerHTML = ""; if(this.value == "'+d.source.id+'"){ var link = $("#userUrl").val() + "?&loc='+d.source.id+'%3A'+d.source.start+'..'+d.source.end+'"; }else{ var link = $("#userUrl").val() + "&loc='+d.target.id+'%3A'+d.target.start+'..'+d.target.end+'"; } linkinput  = document.createElement("a"); linkinput.setAttribute("href",link); linkinput.innerHTML = "Lien Vers JBrowse"; document.getElementById("urlFinal").appendChild(linkinput);');
							
							var option = document.createElement('option');
							option.value = "null"
							option.text= "Select chromosome";
							...
							option.value = d.source.id;
							option.text= d.source.id;
							input.appendChild(option)

							option = document.createElement('option');
							option.value = d.target.id;
							option.text= d.target.id;
							input.appendChild(option)
		...
		_createClass(Heatmap, [{
			...
			value: function renderDatum(parentElement, conf, layout) {
				...
				return parentElement.selectAll('tile').data(function (d) {
					...
					return _this2.theta(d.end, layout.blocks[d.block_id]);
				})).attr('fill', conf.colorValue).on('click',function(d){
					console.log(d);
					$("#modalContent").text("")
					if($("#userUrl").val() !== ""){
						var link = $("#userUrl").val() + "?&loc="+d.block_id+"%3A"+d.start+".."+d.end; 
	...
	var Scatter = function (_Track) {
		...
		_createClass(Scatter, [{
			...
			value: function renderDatum(parentElement, conf, layout) {
				...
				}).attr('stroke', conf.strokeColor).attr('stroke-width', conf.strokeWidth).attr('fill', 'none').on('click',function(d){
					console.log(d);
					$("#modalContent").text("")
					if($("#userUrl").val() !== ""){
						var link = $("#userUrl").val() + "?&loc="+d.block_id+"%3A"+(d.position-1000)+".."+(d.position+1000); 
						window.open(link,"_blank");
					}
				});
	...
	var Stack = function (_Track) {
		...
		_createClass(Stack, [{
			...
			value: function renderDatum(parentElement, conf, layout) {
				...
				}).enter().append('path').attr('class', 'tile').attr('d', (0, _d3Shape.arc)()).attr('opacity', conf.opacity).attr('stroke-width', conf.strokeWidth).attr('stroke', conf.strokeColor).attr('fill', conf.colorValue).on('click',function(d){
					window.open("https://www.ncbi.nlm.nih.gov/protein/"+d.gene,"_blank");
				});
			}
		}]);
